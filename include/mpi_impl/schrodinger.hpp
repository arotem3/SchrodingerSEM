#ifndef MPI_IMPL_SCHRODINGER_HPP
#define MPI_IMPL_SCHRODINGER_HPP
...
namespace schro_mpi
{
    ...
    template <std::floating_point real, std::invocable<SparseData<matrix<real>>, SparseData<matrix<real>>> Callback = NullCallback>
    bool schrodinger_bdf2(SparseData<matrix<real>>& psi_real, SparseData<matrix<real>>& psi_imag, const SparseData<matrix<real>>& potential, const Mesh<real>& mesh, real T, const solver_opts<real>& opts, Callback callback = NullCallback{})
    {
        real alpha = opts.dt;
        ...
        v_wrapper<real> p;
        p.values.resize(2);
        p[0] = std::move(psi_real);
        p[1] = std::move(psi_imag);

        const auto& w = mesh.quadrature.w;

        auto L = [&](const matrix<real>& u, int i) -> matrix<real>
        {
            matrix<real> proj_Vu = (arma::diagmat(w) * (u % potential.at(i)) * arma::diagmat(w)) % mesh.elements.at(i).J;
            matrix<real> nlap = glaplace(u, mesh.elements.at(i), mesh.D, mesh.quadrature);
        ...
        auto Res = [&](const v_wrapper<real>& q) -> v_wrapper<real>
        {
            ...
            for (const auto& [e, element] : mesh.elements)
            {
                matrix<real> u = std::move(y[0].values.at(e));
                matrix<real> v = std::move(y[1].values.at(e));
                matrix<real> proj_u = (arma::diagmat(w) * u * arma::diagmat(w)) % element.J;
                matrix<real> proj_v = (arma::diagmat(w) * v * arma::diagmat(w)) % element.J;
                
                y[0].values.at(e) = proj_u + alpha*L(v, e);
                y[1].values.at(e) = alpha*L(u, e) - proj_v;
            }
        ...
        auto proj = [&mesh](const v_wrapper<real>& p) -> v_wrapper<real>
        {
            ...
            out[1] = real(-1) * solution_wrapper<real>(gproj<real>(p[1].values, mesh));
        ...
        while (t < T)
        {
            ...
            if (ps.size() == 1) {
                rslts = minres<real>(p, Res, ps.at(0), dotprod, IdentityPreconditioner{}, opts.max_iter, opts.tol);
                alpha = real(2.0/3.0) * opts.dt;
            } else {
                v_wrapper<real> b = real(-1.0/3.0)*ps.at(0) + real(4.0/3.0)*ps.at(1);
            ...
            if ((mesh.comm.rank() == 0) and opts.verbose)
                vtracker(t, T);
    ...
    template <std::floating_point real, std::invocable<SparseData<matrix<real>>, SparseData<matrix<real>>> Callback = NullCallback>
    bool schrodinger_dirk4s5(SparseData<matrix<real>>& psi_real, SparseData<matrix<real>>& psi_imag, const SparseData<matrix<real>>& potential, const Mesh<real>& mesh, real T, const solver_opts<real>& opts, Callback callback = NullCallback{})
    {
        ...
        auto f = [&](const v_wrapper<real>& q) -> v_wrapper<real>
        {
            ...
            for (const auto& [e, element] : mesh.elements)
            {
                ...
                y[0].values.at(e) = -L(v, e);
                y[1].values.at(e) = -L(u, e);
            }
        ...
        while (t < T)
        {
            ...
            if (opts.verbose and (mesh.comm.rank() == 0))
    ...
    // https://doi-org.proxy2.cl.msu.edu/10.1007/978-3-662-09947-6_1
    template <std::floating_point real, std::invocable<SparseData<matrix<real>>, SparseData<matrix<real>>> Callback = NullCallback>
    bool schrodinger_dirk4s3(SparseData<matrix<real>>& psi_real, SparseData<matrix<real>>& psi_imag, const SparseData<matrix<real>>& potential, const Mesh<real>& mesh, real T, const solver_opts<real>& opts, Callback callback = NullCallback{})
    {
