...
namespace schro_mpi {
    ...
    template <std::floating_point real>
    std::unordered_map<int, int> distribute_mesh(const Mesh<real>& mesh, int P)
    {
        const int n = mesh.elements.size();

        std::unordered_map<int,int> out;

        std::vector<__distribute_mesh::_idC<real>> a(n);
        int i=0;
        ...
        while (not q.empty()) {
            ...
            __distribute_mesh::PCA1(a, first, last);

            int median_loc = (last + first) / 2;

            std::nth_element(a.begin()+first, a.begin()+median_loc, a.begin()+last, __distribute_mesh::_idcLess());
